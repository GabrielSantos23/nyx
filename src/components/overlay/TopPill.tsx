import { ChevronUp, ChevronDown, Mic, Square } from "lucide-react";
import icon from "../icon.png";
import { TopPillProps } from "../types";
...
export default function TopPill({
  ...
  const toggleListening = async () => {
    if (isListening) {
      await electron?.stopMeeting();
    } else {
      const result = await electron?.startMeeting();
      ...
      >
        ...
        >
          <img
            src={icon}
            alt="Nyx"
            className="w-[28px] h-[28px] object-contain opacity-90 scale-105"
            draggable="false"
