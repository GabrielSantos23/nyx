...
export const GeneralSettings: React.FC<GeneralSettingsProps> = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      if (window.electronAPI?.getRecognitionLanguages) {
        ...
        const stored = localStorage.getItem("nyx_recognition_language");
  ...
  const handleLanguageChange = (key: string) => {
    ...
    localStorage.setItem("nyx_recognition_language", key);
      ...
      <div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-bg-item-surface rounded-xl p-5 border border-border-subtle">
            ...
            <p className="text-xs text-text-tertiary mt-2">
              Select your preferred accent for better recognition accuracy.
            </p>
          </div>
        </div>
      </div>
